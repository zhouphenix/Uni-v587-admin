<template>
	<view class="v587-select" :class="[classList]">
		<picker :mode="mode" :value="index" :range="options" :range-key="rangeKey" :disabled="isDisabled" :end="end"
			:start="start" :fields="fields" @change="onChange" @columnchange="onColumnChange" @cancel="onCancel">
			<view class="v587-select__inner">
				<text class="v587-select__value">{{ text
					}}<text class="v587-select__placeholder" v-if="showPlaceholder">{{
						placeholder
					}}</text></text>
				<text class="v587-select__icon v587-icon-arrow-bottom"></text>
			</view>
		</picker>
	</view>
</template>
...
<script>
	 ...
	 * @example <v587-select :options="[{label: '内容1', value: 1},{label: '内容2', value: 2}]" />
	 */
					...
					switch (this.mode) {
						...
						case "multiSelector":
							return this.index
								...
								.join(this.separator);
						case "region":
							console.warn("请使用 v587-select-region 代替");
						default:
							return this.index;
					}
				})();
