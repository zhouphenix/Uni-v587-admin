<template>
	<v587-popup border-radius="20rpx 20rpx 0 0" :visible.sync="visible" direction="bottom">
		<view class="v587-calendar">
			<view class="v587-calendar__header">
				<text class="v587-calendar__title">{{ title }}</text>
				<text class="v587-calendar__close v587-icon-close" @tap="cancel"></text>
			</view>

			<view class="v587-calendar__container">
				<view class="v587-calendar__selector">
					<!-- 左箭头 -->
					<view class="v587-calendar__selector-icon is-prev" @tap="prevMonth">
						<v587-icon :size="36" name="arrow-left"></v587-icon>
					</view>

					<!-- 当前日期 -->
					<text class="v587-calendar__selector-date">{{ date | format_date }}</text>

					<!-- 右箭头 -->
					<view class="v587-calendar__selector-icon is-next" @tap="nextMonth">
						<v587-icon :size="36" name="arrow-right"></v587-icon>
					</view>

					<!-- 回到今天 -->
					<button class="v587-calendar__today-btn" @tap="toToday">回到今天</button>
				</view>

				<!-- 数据面板 -->
				<view class="v587-calendar__table">
					<view class="v587-calendar__tr">
						<view class="v587-calendar__td">一</view>
						<view class="v587-calendar__td">二</view>
						<view class="v587-calendar__td">三</view>
						<view class="v587-calendar__td">四</view>
						<view class="v587-calendar__td">五</view>
						<view class="v587-calendar__td">六</view>
						<view class="v587-calendar__td">日</view>
					</view>

					<!-- 行 -->
					<view class="v587-calendar__tr" v-for="(tr, index) in table" :key="index">
						<!-- 列 -->
						<view class="v587-calendar__td" v-for="td in tr" :key="td.index" :class="{
							...
							<!-- 日 -->
							<view class="v587-calendar__value">
								{{ td.value }}
							</view>

							<!-- 小点 -->
							<text class="v587-calendar__dot" :style="{
									backgroundColor: td.color,
								}"></text>

							<!-- 备注 -->
							<text class="v587-calendar__remark" v-if="td.remark || td.lunar">
								{{ td.remark || td.lunar }}
							</text>
						</view>
					</view>
			...
			<view class="v587-calendar__footer">
				<v587-row :gutter="20">
					<v587-col :span="12">
						<v587-button fill round @tap="cancel">取消</v587-button>
					</v587-col>

					<v587-col :span="12">
						<v587-button fill round type="primary" :disabled="!isConfirm" @tap="confirm">确定</v587-button>
					</v587-col>
				</v587-row>
			</view>
		</view>
...
<script>
	...
	import dayjs from "../../js_sdk/v587-toolbox/dayjs.js"
	 ...
	 * @example <v587-calendar ref="calendar" v-model="date" />
	 */
