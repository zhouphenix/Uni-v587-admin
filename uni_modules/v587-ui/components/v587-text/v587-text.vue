<template>
	...
	>
		<!-- 价格 -->
		<text class="v587-text__symbol--price" v-if="type == 'price'">￥</text>

		<!-- 前缀图标 -->
		<text class="v587-text__prefix-icon" v-if="prefixIcon">
			<v587-icon :name="prefixIcon" :size="fontSize"></v587-icon>
		</text>

		<text
			class="v587-text__value"
			:style="{
				fontSize,
			}"
			>{{ d.value }}</text
		>
		<text class="v587-text__precision" v-if="type == 'price' && precision"
			>.{{ d.precision }}</text
		>

		<!-- 后缀图标 -->
		<text class="v587-text__suffix-icon" v-if="suffixIcon">
			<v587-icon :name="suffixIcon" :size="fontSize"></v587-icon>
		</text>
	</view>
</template>
 ...
 * @example <v587-text value="Hello, Cool uni" />
 */
