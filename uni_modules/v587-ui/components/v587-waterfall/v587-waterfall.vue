<template>
	...
	>
		...
		<template v-if="direction === 'vertical'">
			<view class="v587-waterfall__item" v-for="(item, index) in list" :key="index">
...
<script>
import { isEmpty } from "../../js_sdk/v587-toolbox/common.js"
...
export default {
	...
	methods: {
		// 刷新列表
		refresh(list) {
			// 清空列表，清空 v587-waterfall-column 的组件遍历
			this.clear();
			...
			this.$nextTick(() => {
				switch (this.direction) {
					case "horizontal":
						...
						// 等待 v587-waterfall-column 渲染完成后追加数据
						this.$nextTick(() => {
							this.append(list);
						});
						break;
		...
		async append(list) {
			for (let i in list) {
				const next = async () => {
					...
					// 获取 v587-waterfall-column 的高度，比较后在最小的列中塞入
		...
		getRect() {
			...
			return new Promise((resolve) => {
				uni.createSelectorQuery()
					.selectAll(`.v587-waterfall-column`)
