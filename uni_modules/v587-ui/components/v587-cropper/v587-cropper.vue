<template>
	<view class="v587-cropper__wrap" v-if="visible" @touchmove.stop.prevent catchtouchmove="false">
		<view class="v587-cropper">
			<view class="v587-cropper__container">
				<!-- 原图片 -->
				<view
					class="v587-cropper__image"
				...
				<!-- 工具 -->
				<view class="v587-cropper__tools">
					<!-- 选择图片 -->
					<view class="v587-cropper__tools-item" @tap="chooseImage">
						<v587-icon name="image" :size="30"></v587-icon>
						<text>选择图片</text>
					</view>

					<!-- 旋转 -->
					<view class="v587-cropper__tools-item" @tap="rotateImage">
						<v587-icon name="refresh" :size="30"></v587-icon>
						<text>旋转</text>
					</view>
				</view>

				<!-- 遮罩层 -->
				<view class="v587-cropper__mask"></view>
			</view>

			<view class="v587-cropper__footer">
		...
		<canvas
			id="canvas"
			canvas-id="canvas"
			class="v587-cropper__canvas"
			:style="[cropStyle]"
		></canvas>

		<!-- Loading -->
		<v587-loading-mask fullscreen :loading="loading"></v587-loading-mask>
		<!-- Toast -->
	...
	methods: {
		...
		setImage(url) {
			...
			uni.createSelectorQuery()
				...
				.in(this)
				// #endif
				.select(".v587-cropper__container")
				.boundingClientRect(res => {
					...
					uni.getImageInfo({
						...
						success: (res) => {
							...
							let dw = (width  - this.image.width) /2
							let dh = (height  - this.image.height) /2
							this.image.left = dw
							this.image.top = dh
					
							// 裁剪框的位置
							this.crop.left = (width  - this.crop.width) /2
							this.crop.top = (height  - this.crop.height) /2
						},
					});
				})
				.exec();
