<template>
	<view class="v587-guide">
		<slot></slot>

		<!-- 遮罩层 -->
		<view
			class="v587-guide__mask"
			:style="{
				'mix-blend-mode': mode,
			}"
			v-if="visible"
		>
			...
			>
				<slot name="tools" :step="step" :current="current">
					...
					<!-- 内容 -->
					<view class="v587-guide__content" v-if="step.content">
						{{ step.content }}
					</view>

					<!-- 按钮组 -->
					<view class="v587-guide__op is-left">
						<!-- 上一步按钮 -->
...
<script>
const { windowHeight, windowTop } = uni.getSystemInfoSync();
/**
 * guide 操作引导
 * @description 步骤引导
...
export default {
	...
	computed: {
		...
		maxHeightLimit() {
			return this.maxHeight || windowHeight
		}
	},
	...
	methods: {
		...
		updateStyle() {
			...
			this.$nextTick(() => {
				uni.createSelectorQuery()
					.in(this)
					.select(".v587-guide__tools")
					.boundingClientRect((res) => {
						
						this.display.style = {
							height: height + "px",
							width: width + "px",
							marginLeft: left - res.left + "px",
							marginTop: top -windowTop + "px",
							"background-color": this.mode == "hard-light" ? "gray" : "#fff",
						};
						
						let _top = top + height -windowTop;
						if (top + height + res.height > this.maxHeightLimit) {
							_top = this.maxHeightLimit - res.height - 20;
						}
