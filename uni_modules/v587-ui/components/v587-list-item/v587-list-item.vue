<template>
	<view
		:class="['v587-list-item', isAppend, isDisabled, isBorder]"
	...
	>
		<view
			class="v587-list-item__swiper"
			:style="{
				transform: translateX,
			}"
		>
			<view class="v587-list-item__container">
				<view class="v587-list-item__icon" v-if="$slots.icon">
					<slot name="icon"></slot>
				</view>

				<text class="v587-list-item__label" v-if="label && label != 'true'">{{ label }}</text>

				<view :class="['v587-list-item__content', isJustify, isColor]">
					<slot></slot>
				</view>

				<view class="v587-list-item__append">
					<slot name="append"></slot>
				</view>
			</view>

			<template v-if="swipe != 'none'">
				<view :class="[`v587-list-item__menu-${swipe}`]">
					<slot name="menu"></slot>
				</view>
			</template>
		</view>
...
export default {
	...
	computed: {
		...
		isAppend() {
			return this.$slots.append ? "v587-list-item--append" : "";
		},

		isDisabled() {
			return this.parent.disabled || this.disabled ? "v587-list-item--disabled" : "";
		},
	...
	methods: {
		...
		setMenu() {
			if (this.swipe != "none") {
				...
				query
					.select(`.v587-list-item__menu-${this.swipe}`)
