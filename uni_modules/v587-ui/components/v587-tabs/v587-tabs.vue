<template>
	<view class="v587-tabs" :class="[classList]">
		<view
			class="v587-tabs__header"
			:style="{
				top: stickyTop
			}"
		>
			<scroll-view
				class="v587-tabs__bar"
			...
			>
				<view
					class="v587-tabs__bar-box"
					:style="{
						'text-align': justify
					}"
				>
					<view
						class="v587-tabs__bar-item"
					...
					>
						...
						<text
							...
							class="v587-tabs__icon v587-tabs__icon--prefix"
							:class="[item.prefixIcon]"
						></text>

						<!-- 文本内容 -->
						<text class="v587-tabs__label">{{ item.label }}</text>

						<!-- 后缀图标 -->
						<text
							v-if="item.suffixIcon"
							class="v587-tabs__icon v587-tabs__icon--suffix"
...
export default {
	...
	computed: {
		...
		isSticky() {
			return this.sticky ? "v587-tabs--sticky" : "";
		},
		...
		onOffset() {
			if (this.tabRect) {
				this.$nextTick(() => {
					...
					if (item) {
						let scrollLeft = item.left - this.offsetLeft - (this.width - item.width) / 2;
