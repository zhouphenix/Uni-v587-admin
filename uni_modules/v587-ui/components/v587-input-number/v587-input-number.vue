<template>
	<view class="v587-input-number">
		<!-- 减 -->
		<button class="v587-input-number__minus" @tap="onMinus">
			<text class="v587-icon-minus"></text>
		</button>

		<!-- 值 -->
		<view class="v587-input-number__value" :style="{ width: width2 }">
			<input type="number" v-model="value2" @change="update" v-if="input" />
			<text v-else>{{ value2 }}</text>
		</view>

		<!-- 加 -->
		<button class="v587-input-number__plus" @tap="onPlus">
			<text class="v587-icon-plus"></text>
		</button>
	</view>
</template>
 ...
 * @example <v587-input-number v-model="val" />
 */

import { parseRpx, isString, isDecimal } from "../../js_sdk/v587-toolbox/common.js"

export default {
	name: "v587-input-number",
