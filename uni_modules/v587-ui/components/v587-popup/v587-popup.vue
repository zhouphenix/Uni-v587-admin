<template>
	<view
		...
		:class="[
			'v587-popup__wrapper',
			`v587-popup__wrapper--${direction}`,
			`is-${status ? 'open' : 'close'}`,
		]"
		@touchmove.stop.prevent
	>
		<view class="v587-popup__modal" @tap="modalClose"></view>

		<view
			:class="['v587-popup']"
			:style="{ height, width, backgroundColor, borderRadius: parseRpx(borderRadius) }"
		>
			<view class="v587-popup__container" :style="{ padding: parseRpx(padding) }">
				<slot></slot>
			</view>
		</view>
	</view>
 ...
 * @example <v587-popup :visible.sync="visible">Hello !</v587-popup>
 */
