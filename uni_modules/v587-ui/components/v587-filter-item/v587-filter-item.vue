<template>
	<view class="v587-filter-item" :class="[classList]" @tap="tapItem">
		<!-- 文本内容 -->
		<text class="v587-filter-item__label">{{ name || label }}</text>

		<!-- 下拉菜单 -->
		<template v-if="type == 'dropdown'">
			<text class="v587-icon-caret-bottom v587-filter-item__dropdown-icon"></text>
		</template>
		...
		<template v-else-if="type == 'order'">
			<view class="v587-filter-item__order" :class="[`is-${order}`]" v-if="order !== undefined">
				<!-- 升序，降序图标 -->
				<text class="v587-filter-item__order-asc"></text>
				<text class="v587-filter-item__order-desc"></text>
			</view>
		</template>

		<!-- 展开内容 -->
		<view class="v587-filter-item__dropdown-box" :class="[`theme-${theme}`]">
			<slot name="dropdown">
				<template v-if="list">
					<!-- 数据 -->
					<template v-if="list.length > 0">
						<!-- 列表 -->
						<scroll-view class="v587-filter-item__dropdown-box__list" scroll-y>
							<view
								class="v587-filter-item__dropdown-box__item"
								:class="[
							...
							>
								<text class="v587-filter-item__dropdown-box__label">{{
									item.label
								}}</text>
								<text class="v587-icon-check"></text>
							</view>
						</scroll-view>

						<!-- 多选下的操作按钮 -->
						<view class="v587-filter-item__dropdown-box__btn" v-if="multiple">
							<v587-row :gutter="30">
								<v587-col :span="12">
									<v587-button fill round @tap="clear">清空</v587-button>
								</v587-col>

								<v587-col :span="12">
									<v587-button fill round type="primary" @tap="confirm"
										>确认</v587-button
									>
								</v587-col>
							</v587-row>
					...
					<template v-else>
						<slot name="empty">
							<view class="v587-filter-item__dropdown-box__empty"> 暂无数据 </view>
						</slot>
					</template>
				</template>
			</slot>
