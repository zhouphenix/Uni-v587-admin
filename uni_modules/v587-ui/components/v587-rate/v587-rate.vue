<template>
	<view class="v587-rate">
		<view class="v587-rate__icon" v-for="(item, index) in list" :key="index" @tap="onTap(index)">
			<v587-icon :name="icon" :size="size" :color="voidColor"></v587-icon>

			<view
				class="v587-rate__icon-active"
				:style="{
					width: item.width,
				}"
			>
				<v587-icon :name="icon" :size="size" :color="item.color"></v587-icon>
			</view>
		</view>

		<view class="v587-rate__text" v-if="showText">
			{{ value2 }}
		</view>
	</view>
</template>
...
<script>
import { isNumber, getCurrentColor } from "../../js_sdk/v587-toolbox/common.js"
 ...
 * @property {String} icon 评分图标，默认v587-icon-favor-fill
